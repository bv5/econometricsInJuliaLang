...
function generate_results(Fname, variation_threshold=10, minimum_row_count=10, bunch_prices=false)
    ...
    if occursin(r"\.xlsx$"i, Fname)
        ...
        for row in 2:total_rows
            ...
            if price == 0
                println("Found zero price with qty " * string(qty))
                continue
            end
...
if length(ARGS) >= 1
    ...
    if isdir(ARGS[1])
        ...
        for fname in files_list
            if occursin(r"^results_"i, fname) || occursin(r"^\.", fname)
                ...
                if !isdir(ARGS[1] * "//$fname")
                    generate_results(ARGS[1] * "//$fname", thresholdVariation, minimumRows, bunch_prices)
                else
        ...
        generate_results(ARGS[1], thresholdVariation, minimumRows, bunch_prices)
    end
else
    println("\n" * "=" ^ 80 * """
    \nPlease run the program like this:\n\njulia pq.jl <excel/csv file or folder containing excel/csv files> <variation threshold> <minimum number of rows to be present in file> <bunchprices>\n\nNote: 1st argument is mandatory, the other three arguments are optional""" * "\n" * "=" ^ 80)
    exit()
end
